...
- 0011: ifc/iful/ifungeq
  - if status.carry = 1
- 0100: ifung/ifuleq
  - if status.carry = 1 or status.zero = 1
- 0101: ifnc/ifunl/ifugeq
  - if status.carry = 0
- 0110: ifo
...
#### 3.3.1.6. A.1r.1s - Conditional 1 GP Integer Register, 1 Special Register
...
#### 3.4.2.1. 00101: `add4` - (A.4r) - Add Register2 and Register3 Storing the Lower 32-bits of the Sum in Register0 and the Upper 32-bits of the in Register1
...
#### 3.4.2.2. 00110: `uadd` - (A.3r.1imm8) - Add Register1, Register2 and Unsigned8 Storing the Lower 32-bits of the Sum in Register0

- if condition? register0 = (register1 + register2 + unsigned8) & 0xffffffff

#### 3.4.2.4. 00111: `sub3` - (A.3r) - Subtract Register2 from Register1 Storing the Difference in Register0

- if condition? register0 = (register1 - register2)

#### 3.4.2.3. 01000: `sub` - (A.2r.1imm12) - Sub Unsigned12 from Register1 Storing the Difference in Register0

- if condition? register0 = (register1 - unsigned12) & 0xffffffff

#### 3.4.2.5. 01001: `mul4` - (A.4r.flag1) - Multiply Register2 by Register3 Storing the Lower 32-bits of the Product in Register0 and the Upper 32-bits of the Product in Register1
...
#### 3.4.2.6. 01010: `umul` - (A.2r.1imm12) - Multiply Register1 by Unsigned12 Storing the Lower 32-bits of the Product in Register0

- if condition? register0 = (register1 * unsigned12) & 0xffffffff

#### 3.4.2.8. 01011: `div4` - (A.4r.flag1) - Divide Register2 by Register3 Storing the Quotient in Register0 and the Remainder in Register1
...
#### 3.4.2.9. 01100: `udiv` - (A.2r.1imm12) - Divide Register1 by Unsigned12 Storing the Quotient in Register0

- if condition? register0 = (register1 / unsigned12)

### 3.4.3. Logic

#### 3.4.3.1. 01101: `and4` - (A.4r.flag1) - Binary And Register2 with Register3 Storing the Result in Register0 and the Result Negated in Register1
...
#### 3.4.3.2. 01110: `and` - (A.2r.1imm12.flag1) - Binary And Register1 with Immediate12 Storing the Result in Register0

- if condition and flag1 is 0? register0 = (register1 & immediate12)
- if condition and flag1 is 1? register0 = ~(register1 & immediate12)

#### 3.4.3.3. 01111: `or4` - (A.4r.flag1) - Binary Or Register2 with Register3 Storing the Result in Register0 and the Result Negated in Register1
...
#### 3.4.3.4. 10000: `or` - (A.2r.1imm12.flag1) - Binary Or Register1 with Immediate12 Storing the Result in Register0

- if condition and flag1 is 0? register0 = (register1 | immediate12)
- if condition and flag1 is 1? register0 = ~(register1 | immediate12)

#### 3.4.3.5. 10001: `xor4` - (A.4r.flag1) - Binary Exclusive Or Register2 with Register3 Storing the Result in Register0 and the Result Negated in Register1
...
#### 3.4.3.6. 10010: `xor` - (A.2r.1imm12.flag1) - Binary Exclusive Or Register1 with Immediate12 Storing the Result in Register0

- if condition and flag1 is 0? register0 = (register1 ^ immediate12)
- if condition and flag1 is 1? register0 = ~(register1 ^ immediate12)

#### 3.4.3.7. 10011: `sh4` - (A.4r.flag1) - Shift Register2 Left/Right by Register3 bits Storing the Result in Register0 and the Overflow/Underflow of the Result in Register1
...
#### 3.4.3.8. 10100: `sh` - (A.2r.1imm12.flag1) - Shift Register1 Left/Right by Unsigned12 Storing the Result in Register0 and the Overflow/Underflow of the Result in Register1
...
- if condition?
  - if flag3.2 is 0?
    ...
    - if flag3.1 is 1? register0 = int(float0)
  - if flag3.2 is 1?
...
#### 3.4.6.2. 11001: `fas` - (A.6f.flag1) - Add/Subtract Floats
...
#### 3.4.6.3. 11010: `fmul` - (A.6f) - Multiply Floats

- if condition? float0 = float1 * float2; float3 = float4 * float5
- NOTE: if float0 and float3 are the same, the second operation is discarded and float0/float3 is equal to float1 * float2

#### 3.4.6.4. 11011: `fdiv` - (A.7f) - Divide Floats
...
#### 3.4.6.7. 11100: `fsqrt` - (A.6f) - Perform Square Root on Floats
...
#### 3.4.6.8. 11101: `flog` - (A.6f) - Perform Logarithm on Floats
...
#### 3.4.6.9. 11110: `fpow` - (A.6f) - Perform Exponentiation on Floats
...
#### 3.4.6.10. 11111: `ftype` - (A.1f) - Determine Type of Float by Setting Status Flags
